...
def read_file(fname):
    required_cols = ["TheTime","CH1_Hz","Inlet_BLC","NOx_cal","Inlet_NOx","Inlet_ZA","NO_Cal_flow","Inlet_SampleFlow_1","NO_valve","zero_valve_1", "OxygenFlow_1", "PMT_T", "Rxn_Cell_T", "Rxn_Vessel_Pressure","Inlet_SS"]
    df = pd.read_csv(fname,usecols = required_cols,index_col = "TheTime",on_bad_lines = "skip")
    return df
...
def pd_stats(df:pd.DataFrame,key:str,stat_name:str,delay:float,agg_col:str = "NOx_counts",period_key:str = "",take_last_in_period = False) -> pd.DataFrame:
    return df.pipe(lambda a: a if period_key == "" else a.loc[df[period_key] == True])[[x for x in ["TheTime",key + "_iter",agg_col,period_key + "_iter"] if x != "_iter"]]\
                          ...
                          .pipe(lambda z: z if take_last_in_period == False else z.groupby(period_key + "_iter",as_index = False).nth(-1))\
                          .set_index("TheTime")
...
def run_me(curr_folder,data_folder,output_folder,year):
    ...
    df["TheTime"] = df.index.values
    df.index = pd.DatetimeIndex(pd.to_datetime(df.index, unit='D', origin='1899-12-30'))
    ...
    calc_df.index = pd.to_datetime(calc_df.index, unit='D', origin='1899-12-30')
    ...
    cal_df.index = pd.to_datetime(cal_df.index, unit='D', origin='1899-12-30')

    art_df = pd.concat([pd_stats(df,key,key,delay) for key,delay in artifact_stats],axis = 1).sort_index()
    art_df.index = pd.to_datetime(art_df.index, unit='D', origin='1899-12-30')
    ...
    calc_df.to_csv(output_folder + os.path.sep + "NOx_kcg_og_" + str(year) + "_calc_df.csv")
...
if __name__ == "__main__":
    ...
    data_folder = os.path.join(curr_folder,"nox_raw_data/raw")
    output_folder = os.path.join(curr_folder,"processed_data")
    #years = [2022]
